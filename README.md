...
* вопросы к боту — упоминание (`@GPTipsBot …`) или reply на сообщение бота; ответ виден всем в чате, списание с пользователя;
* `/image` — генерация картинок (видны всем, баланс списывается с автора);
* `/summary` — краткое содержание сообщений за сегодня (в чат).

Остальные команды в группах недоступны — ими пользуйтесь в личке с ботом.

Реклама в группах не отправляется.

В меню группы Telegram показывает только `/image` и `/summary` (через `BotCommandScope.AllGroupChats` + меню на конкретный чат). Полный список команд — в личке.
