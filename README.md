...
Бот отвечает в группах и супергруппах, когда к нему обращаются:

* командой (`/help`, `/help@GPTipsBot`, …);
* упоминанием (`@GPTipsBot вопрос`);
* ответом (reply) на сообщение бота;
* в рамках уже начатого сценария (картинка, OCR, депозит и т.п.).

В группах не показываются reply-клавиатуры — только команды и inline-кнопки. Реклама в группах не отправляется.

В группах бот отвечает в личные сообщения. Чтобы это работало, сначала напишите боту /start в личке.

Команда `/summary` — краткое содержание сообщений за сегодня (в группе — по сообщениям, которые бот видит).

При **включённом privacy mode** (по умолчанию в BotFather) Telegram и так присылает боту только команды, упоминания и reply. Чтобы бот видел все сообщения группы для `/summary`, выключите Group Privacy у `@BotFather` → `/setprivacy` → Disable — код всё равно ответит только при обращении.
