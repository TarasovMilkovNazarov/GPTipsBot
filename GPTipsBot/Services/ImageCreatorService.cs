...
public class ImageCreatorService
{
    ...
    public ImageCreatorService(ILogger log) {
        this.log = log;
    ...
    {
        var newClient = new RestClient(new RestClientOptions(BingUrl) {
        ...
        newClient.AddDefaultHeader("origin", BingUrl);
        newClient.AddDefaultHeader("x-forwarded-for", $"13.{random.Next(104, 108)}.{random.Next(0, 256)}.{random.Next(0, 256)}"); // Generate random IP between range 13.104.0.0/14
    ...
    public async Task<List<string>> GetImageSources(string prompt, CancellationToken token)
    {
        ...
        try
        {
            ...
            // Get redirect URL
            var redirectUrl = response.Headers?.First(x => x.Name == "Location").Value?.ToString()?.Replace("&nfy=1", "");
